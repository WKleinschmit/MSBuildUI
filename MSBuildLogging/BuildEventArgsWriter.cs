...
namespace MSBuildLogging
{
    ...
    internal class BuildEventArgsWriter
    {
            ...
            switch (e)
            {
                ...
                default:
                    ...
                    BuildMessageEventArgs buildMessageEventArgs = new BuildMessageEventArgs(
                        ...
                        e.Timestamp) {BuildEventContext = e.BuildEventContext ?? BuildEventContext.Invalid};
        ...
        private void WriteItemList(IEnumerable items)
        {
            ...
            {
                IEnumerable<ITaskItem> enumerable = taskItems as ITaskItem[] ?? taskItems.ToArray();
                Write(enumerable.Count());

                foreach (ITaskItem item in enumerable)
                {
                    Write(item);
                }
        ...
        private void WriteProperties(IEnumerable properties)
        {
            ...
            {
                if (entry.Key is string s1 && entry.Value is string s2)
                {
                    Write(s1);
                    Write(s2);
                }
        ...
        {
            IEnumerable<KeyValuePair<TKey, TValue>> valuePairs = keyValuePairs as KeyValuePair<TKey, TValue>[] ?? keyValuePairs.ToArray();
            if (valuePairs.Any())
            {
                Write(valuePairs.Count());
                foreach (KeyValuePair<TKey, TValue> kvp in valuePairs)
                {
